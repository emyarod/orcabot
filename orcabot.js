...
bot.addListener('message', function(from, to, text, message) {
	...
	// search for a colon in input command
	if (text.search(/:/g) > 0) {
		// '.tr code:code your text here'
		if (text.search(/(\.)(tr)( ).*?(:).*?( )/g) === 0) {
			// '.tr code:code '
			...
			translator.getLanguageNames(params, function(e, data) {
				if (!e) {
					inputLanguage = data[0];
					outputLanguage = data[1];
	...
	} else if (text.search(detectToEnglish) === 0) {
		// ".tr word or sentence", defaults to English output
		var found = text.match(detectToEnglish);
		...
		translator.detect(params, function(e, data) {
			if (!e) {
				params.from = data;
					...
					if (!e) {
						...
						translator.translate(params, function(e, translatedText) {
							if (!e) {
								...
								translator.speak(params, function(e, data) {
									if (!e) {
